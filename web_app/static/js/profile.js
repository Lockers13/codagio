    ...
    toggle.addEventListener('click', function() {
        ...
        if(toggle.value == 0) {
            ...
            for(let i = 0; i < stats_list.length; i++) {
                ...
                build_string += "<button class='btn btn-link' data-toggle='collapse' data-target='#collapse" + i + "' aria-expanded='true' aria-controls='collapse" + i +"'>" + stats_list[i]["problem__name"] + "</button></h5></div>"
            ...
            for(let i = 0; i < stats_list.length; i++) {
                ...
                build_string += "<div class='card-header' id='heading" + i +"'>"
                build_string += "<h5 class='mb-0'>"
                build_string += "<button class='btn btn-link' data-toggle='collapse' data-target='#collapse" + i + "' aria-expanded='true' aria-controls='collapse" + i +"'>" + stats_list[i]["name"] + "</button></h5></div>"
                build_string += "<div id='collapse" + i +"' class='collapse' aria-labelledby='heading" + i + "' data-parent='#accordion'>"
                build_string += "<div class='card-body'>"
